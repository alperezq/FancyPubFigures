...
Author_lastname<-c("Perez-Quintero","Pérez-Quintero") #last name and alternative spellings

#Get publication record from scholar
SC<-gcite(user=Scholar_ID,plot_wordcloud = FALSE)
df<-SC$paper_df
#I reccomend saving this data frame and reading it back, since the gcite function wil throw errors after using it multiple times
write.csv(df,"Scholar_citations.csv")
df<-read.csv("Scholar_citations.csv")
...
``` r
#for this section we use the SC object obtainedby doing SC<-gcite(user=Scholar_ID,plot_wordcloud = FALSE) 

hind<-SC$citation_indices[2,2]
...
#all plots same size
plot_grid(p1v1,p2,p3,p4,ncol=2,nrow=2)
```
...
allpanels<-plot_grid(p1v1,panel2,ncol=2,rel_widths = c(1.5,2))
